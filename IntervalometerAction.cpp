...
IntervalometerAction:: left()
{
    if (not _shooting)
    {
        ...
        case FOCUS_SHOOT_DELAY:
			...
			if (_focusShootDelay > 100)
			{
				_focusShootDelay -= 100;
			}
			else if (_focusShootDelay > 10)
			{
				_focusShootDelay -= 10;
			}
			else if (_focusShootDelay > 0)
			{
				_focusShootDelay -= 1;
			}
...
IntervalometerAction:: right()
{
    if (not _shooting)
    {
        ...
        case FOCUS_SHOOT_DELAY:
			...
			if (_focusShootDelay < 10)
			{
				_focusShootDelay += 1;
			}
			else if (_focusShootDelay < 100)
			{
				_focusShootDelay += 10;
			}
			else if (_focusShootDelay < 1000)
			{
				_focusShootDelay += 100;
			}
...
void IntervalometerAction:: action()
{
	display();
	...
	if (_shotTimeCount >= _shotInterval)
	{
		...
		// FIXME Take shot here

		digitalWrite(13, HIGH);
		if (_focusShootDelay > 0)
		{
			delay(_focusShootDelay);
		}
		digitalWrite(13, LOW);
...
IntervalometerAction:: display()
{
	...
	else
	{
        ...
        case FOCUS_SHOOT_DELAY:
			...
			_lcd.print(_focusShootDelay, DEC);
			_lcd.print(" ms");
            break;
